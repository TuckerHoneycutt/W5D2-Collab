...
const convertToBase2 = element => {
  //Check to see if element is a string and if it starts with '0x'
  if(typeof element === 'string' && element.startsWith('Ox')){
    return '0b' +(parseInt(element, 16).toString(2));
  }else if(typeof element === 'string' && element.startsWith('0b')){    //Check to see if element is a string and if it starts with '0b'

    return element;
  }
  else{  //Otherwise, parse the input as an integer and convert it to binary
